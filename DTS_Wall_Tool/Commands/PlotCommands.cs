...
namespace DTS_Wall_Tool.Commands
{
    ...
    public class PlotCommands : CommandBase
    {
        ...
        public void DTS_PLOT_FROM_SAP()
        {
            ...
            // Bước 1: Lấy danh sách tầng từ SAP
            ...
            // Bước 2: Hiển thị menu chọn tầng
            ...
            // Bước 3: Nếu chọn "All", hỏi chế độ 2D/3D
            bool is3D = false;
        ...
        private void PlotSingleStory2D(SapUtils.GridStoryItem story)
        {
            ...
            // Bước 1: Pick điểm đặt gốc trong CAD
            ...
            // Bước 2: Lấy dữ liệu grid và frame từ SAP
           ...
           .Where(f => Math.Abs(f.AverageZ - storyElev) <= 200)
            ...
            UsingTransaction(tr =>
            {
                ...
                // Vẽ frame
                PlotFramesAt(btr, tr, framesAtStory, insertPoint, story.Coordinate);

                // Tạo Origin
                CreateOriginForStory(btr, tr, insertPoint, story);
            });
        ...
        private void PlotAll3D(List<SapUtils.GridStoryItem> stories)
        {
            ...
            UsingTransaction(tr =>
            {
                ...
                .ToList();
                    ...
                    // Vẽ frame với tọa độ Z đúng (3D)
                    int beforeCount = 0;
                    try { beforeCount = framesAtStory.Count; } catch { }
                    PlotFramesAt3D(btr, tr, framesAtStory, insertPoint, story.Coordinate);
                    ...
                    // Tạo Origin tại Z của tầng
                    CreateOriginForStory3D(btr, tr, insertPoint, story);
                }

                // Vẽ lưới trục chỉ ở mặt bằng đầu tiên (tầng thấp nhất)
                var firstStory = sortedStories.First();
                PlotGridLines3D(btr, tr, allGrids, baseInsertPoint, firstStory.Coordinate);
            });

            WriteSuccess("Vẽ mô hình 3D hoàn thành");
        }
                    ...
                    List<SapUtils.GridLineRecord> grids, Point2D offset, double elevation)
        {
            ...
            double minX = xGrids.Min(g => g.Coordinate);
            double maxX = xGrids.Max(g => g.Coordinate);
            ...
            double extendLength = maxRange * 0.5;
            double extMinX = minX - extendLength;
            double extMaxX = maxX + extendLength;
            double extMinY = minY - extendLength;
            double extMaxY = maxY + extendLength;

            // Vẽ X-axis
            foreach (var xGrid in xGrids)
            {
                Point3d pt1 = new Point3d(xGrid.Coordinate + offset.X, extMinY + offset.Y, 0);
                Point3d pt2 = new Point3d(xGrid.Coordinate + offset.X, extMaxY + offset.Y, 0);
                ...
                PlotAxisLabel(btr, tr, xGrid.Name, new Point3d(pt1.X, extMaxY + offset.Y + 100, 0));
                PlotAxisLabel(btr, tr, xGrid.Name, new Point3d(pt1.X, extMinY + offset.Y - 100, 0));
            }

            // Vẽ Y-axis
            foreach (var yGrid in yGrids)
            {
                Point3d pt1 = new Point3d(extMinX + offset.X, yGrid.Coordinate + offset.Y, 0);
                Point3d pt2 = new Point3d(extMaxX + offset.X, yGrid.Coordinate + offset.Y, 0);
                ...
                PlotAxisLabel(btr, tr, yGrid.Name, new Point3d(extMinX + offset.X - 200, pt1.Y, 0));
                PlotAxisLabel(btr, tr, yGrid.Name, new Point3d(extMaxX + offset.X + 100, pt1.Y, 0));
            }
        }

        /// <summary>
        /// Vẽ frame với tọa độ 3D (Z từ frame chính)
        /// </summary>
        private void PlotFramesAt3D(BlockTableRecord btr, Transaction tr, List<SapFrame> frames, Point2D offset, double elevation)
        {
            foreach (var frame in frames)
            {
                ...
                // Use centralized LabelUtils for labels
        ...
        /// <summary>
        /// Tạo Origin tại 3D (với cao độ Z)
        /// </summary>
        private void CreateOriginForStory3D(BlockTableRecord btr, Transaction tr,
        ...
        private void PlotGridLines(BlockTableRecord btr, Transaction tr, List<SapUtils.GridLineRecord> grids, Point2D offset)
        {
            ...
            double extend = maxRange * 0.5;
            double extMinX = minX - extend;
            double extMaxX = maxX + extend;
            double extMinY = minY - extend;
            double extMaxY = maxY + extend;
        ...
        /// </summary>
        private void PlotFramesAt(BlockTableRecord btr, Transaction tr, List<SapFrame> frames, Point2D offset, double elevation)
        {
        ...
        /// <summary>
        /// Tạo Origin cho mặt bằng2D
        /// </summary>
        private void CreateOriginForStory(BlockTableRecord btr, Transaction tr, Point2D insertPoint, SapUtils.GridStoryItem story)
        {
            AcadUtils.CreateLayer(ORIGIN_LAYER, 1);
        ...
        private void PlotAllStories2D(List<SapUtils.GridStoryItem> stories)
        {
            ...
            // Compute max gap between adjacent Y grid lines and use spacing =2 * maxGap
            double spacing = 0;
            if (yGrids.Count >= 2)
            {
                double maxGap = 0;
                ...
                {
                    double gap = Math.Abs(yGrids[i].Coordinate - yGrids[i - 1].Coordinate);
                    if (gap > maxGap) maxGap = gap;
                }
                spacing = Math.Max(1000.0, maxGap * 2.0); // ensure reasonable min spacing
            }
            else
            {
                spacing =10000.0; // fallback
            }

            // Prompt user for base insertion point for All-2D layout
            WriteMessage($"\nChọn điểm gốc để vẽ bố cục {stories.Count} mặt bằng (All2D).\nKhoảng cách giữa mặt bằng = {spacing:0} mm");
            ...
            UsingTransaction(tr =>
            {
                ...
                for (int i =0; i < sorted.Count; i++)
                {
                    ...
                    PlotFramesAt(btr, tr, framesAt, insert, st.Coordinate);
                    CreateOriginForStory(btr, tr, insert, st);
                }
            });
